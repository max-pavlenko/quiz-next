...
const SubscriptionPlans: FC<Props> = ({onChange, plan}) => {
       ...
       <section className="grid gap-4">
          ...
          <div className="grid gap-2">
             {PLANS.map(({text, price, isPopular}) => (
                 <PlanOption key={text} onChange={handlePlanChange} checked={text === plan} isPopular={isPopular} price={price} value={text}>
                    {text}
                 </PlanOption>
             ))}
          </div>
          <Button>Get my plan</Button>
          <p className="text-[10px] text-center text-white-400">
             You are enrolling in a {plan} subscription to
             &nbsp;<Link className="font-medium text-text" href="/public">https://awesomeapp.com/subscription</Link> with the discount price
